...
tags: [kiro-spec, workflow, status, progress]
category: kiro-spec
metadata:
  version: "1.0"
...
- Invoked as: `/kiro-spec-status <feature-name>`

## Steps

### 1. Locate the Spec

- Check `.kiro/specs/<feature-name>/spec.json`.
- If not found, report: "No spec found for `<feature-name>`. Run `/kiro-spec-init` to start."
...
|-----------|----------------|
| Phase = initialized, requirements not generated | Run `/kiro-spec-requirements <feature-name>` |
| Requirements generated, not approved | Review requirements.md and approve |
| Requirements approved, design not generated | Run `/kiro-spec-design <feature-name>` |
| Design generated, not approved | Optional: run `/kiro-validate-design <feature-name>`, then approve |
| Design approved, tasks not generated | Run `/kiro-spec-tasks <feature-name>` |
| Tasks generated, not approved | Review tasks.md and approve |
| ready_for_implementation = true, tasks incomplete | Run `/kiro-spec-impl <feature-name>` |
| All tasks complete | Run `/kiro-validate-impl <feature-name>` |
| Phase = complete | 🎉 Spec complete! |

## Guardrails

- This command is read-only — it never modifies `spec.json` or any artifact.
- Always list all specs under `.kiro/specs/` if no feature name is provided:
  ```
  Available specs:
...
- [`kiro-spec-init`](../kiro-spec-init/SKILL.md) — start a new spec
- [`kiro-spec-impl`](../kiro-spec-impl/SKILL.md) — implementation phase
- [`kiro-validate-impl`](../kiro-validate-impl/SKILL.md) — post-implementation check

...
tags: [spec, workflow, design, architecture]
category: spec
metadata:
  version: "1.0"
  source: .agents/
...
Produces `.agents/specs/<feature-name>/design.md` covering architecture, components, interfaces, data models, error handling, and testing strategy. Optionally generates `research.md` for detailed investigation notes. Requires human approval before the spec advances to task generation.
...
- Invoked as: `/spec-design <feature-name>` or `/spec-design <feature-name> -y`

## Steps

### 1. Load Context

- Read `.agents/specs/<feature-name>/requirements.md` (approved requirements).
- Read all files under `.agents/steering/` as project memory.
...
- [`spec-validate-design`](../spec-validate-design/SKILL.md) — optional design quality review
- [`spec-tasks`](../spec-tasks/SKILL.md) — next step after design approval

...
tags: [spec, workflow, implementation, execution]
category: spec
metadata:
  version: "1.0"
  source: .agents/
...
- Invoked as:
  - `/spec-implement <feature-name>` — implement all remaining tasks
  - `/spec-implement <feature-name> 1.1 1.2` — implement specific task numbers only
...
- Read `.agents/specs/<feature-name>/tasks.md` — the task list
- Read `.agents/specs/<feature-name>/design.md` — architecture and interface contracts
- Read `.agents/specs/<feature-name>/requirements.md` — acceptance criteria
...
4. **Implement** the code:
   ...
   - Use naming conventions from `.agents/steering/` files.
...
- Recommend running `/spec-validate-implementation <feature-name>` to verify correctness.
...
- [`spec-validate-implementation`](../spec-validate-implementation/SKILL.md) — validation after implementation
- [`spec-status`](../spec-status/SKILL.md) — check progress
- [`spec-tasks`](../spec-tasks/SKILL.md) — if tasks need revision

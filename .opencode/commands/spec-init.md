...
## Execution Steps
1. **Check Uniqueness**: Verify `.agents/specs/` for naming conflicts (append number suffix if needed)
2. **Create Directory**: `.agents/specs/[feature-name]/`
3. **Initialize Files Using Templates**:
   - Read `.agents/templates/specs/init.json`
   - Read `.agents/templates/specs/requirements-init.md`
...
- **Template Missing**: If template files don't exist in `.agents/templates/specs/`, report error with specific missing file path and suggest checking repository setup

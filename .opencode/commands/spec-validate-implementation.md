...
- Parse conversation history for `/spec-implement <feature> [tasks]` commands
...
- If no history found, scan `.agents/specs/` for features with completed tasks `[x]`

**If feature provided** (`$1` present, `$2` empty):
- Use specified feature
- Detect all completed tasks `[x]` in `.agents/specs/$1/tasks.md`
...
- Read `.agents/specs/<feature>/spec.json` for metadata
- Read `.agents/specs/<feature>/requirements.md` for requirements
- Read `.agents/specs/<feature>/design.md` for design structure
- Read `.agents/specs/<feature>/tasks.md` for task list
- **Load ALL steering context**: Read entire `.agents/steering/` directory including:
...
## Tool Guidance
- **Conversation parsing**: Extract `/spec-implement` patterns from history
...
### Error Scenarios
- **No Implementation Found**: If no `/spec-implement` in history and no `[x]` tasks, report "No implementations detected"
...
- Re-run `/spec-implement <feature> [tasks]` for fixes
- Re-validate with `/spec-validate-implementation [feature] [tasks]`

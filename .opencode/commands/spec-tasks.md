...
- `.agents/specs/$1/spec.json`, `requirements.md`, `design.md`
- `.agents/specs/$1/tasks.md` (if exists, for merge mode)
...
- Read `.agents/rules/tasks-generation.md` for principles
- If `sequential` is **false**: Read `.agents/rules/tasks-parallel-analysis.md` for parallel judgement criteria
- Read `.agents/templates/specs/tasks.md` for format (supports `(P)` markers)
...
**Write and update**:
- Create/update `.agents/specs/$1/tasks.md`
...
1. **Status**: Confirm tasks generated at `.agents/specs/$1/tasks.md`
2. **Task Summary**: 
...
- **Suggested Action**: "Run `/spec-tasks $1 -y` to auto-approve both and proceed"

**Missing Requirements or Design**:
- **Stop Execution**: Both documents must exist
- **User Message**: "Missing requirements.md or design.md at `.agents/specs/$1/`"
...
- **User Message**: "Template or rules files missing in `.agents/`"
...
- **IMPORTANT**: Clear conversation history and free up context before running `/spec-implement`
- This applies when starting first task OR switching between tasks
- Fresh context ensures clean state and proper task focus

**If Tasks Approved**:
- Execute specific task: `/spec-implement $1 1.1` (recommended: clear context between each task)
- Execute multiple tasks: `/spec-implement $1 1.1,1.2` (use cautiously, clear context between tasks)
- Without arguments: `/spec-implement $1` (executes all pending tasks - NOT recommended due to context bloat)

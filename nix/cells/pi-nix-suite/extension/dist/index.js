...
export default async function (pi) {
    ...
    // Initialize managers
    const tmux = new TmuxManager(socketPath, config.tmux.sessionName, config.tmux.windowPrefix);
    ...
    pi.on('session_complete', async (event) => {
        if (!config.skillGeneration.enabled)
            return;
        const suggestion = skillGenerator.analyzeSession(event.messages, event.workingDir);
        if (suggestion) {
            // Prompt user with one-keystroke approval
            pi.notify('info', `📝 Create skill '${suggestion.name}' from this session? [Y/n/skip]`, { actions: ['Y', 'n', 'skip'] });
        }
    });
